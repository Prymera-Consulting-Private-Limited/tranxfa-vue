...
<template>
  <div class="relative flex-1 flex items-center justify-center p-4 md:p-8">
    ...
    <div v-show="! showLoading" class="w-full max-w-xl">
      ...
      <!-- Form Header -->
      <h2 class="text-2xl font-semibold text-black mb-4 mt-14 sm:mt-8">{{ $t('onboarding.personalDetails') }}</h2>
      <div class="text-sm/6 text-gray-900 mb-8 flex items-center justify-start gap-x-1">{{ $t('onboarding.signedUpFrom') }}<FlagIcon v-if="customer?.data?.country" :class="['text-xl border border-gray-100']" :code="customer?.data?.country?.iso2Alpha.toLowerCase()" circle  /> <span class="font-semibold">{{ customer?.data?.country?.commonName }}</span>
        <a class="text-brand-700 hover:underline" @click="changeCountry" href="javascript:">{{ $t('onboarding.change') }}</a></div>
      <!-- Form -->
