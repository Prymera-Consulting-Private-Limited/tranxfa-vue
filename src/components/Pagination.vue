...
<template>
  ...
  <nav :aria-label="$t('account.pagination')" class="flex items-center justify-between border-t border-gray-200">
    <div class="-mt-px flex w-0 flex-1">
      ...
      >
        <ArrowLongLeftIcon :class="[pagination.links.prev ? 'text-gray-400' : 'text-gray-300']" class="mr-3 size-5" aria-hidden="true" />{{ $t('account.previous') }}</button>
    </div>
    ...
    <div class="-mt-px flex w-0 flex-1 justify-end">
      ...
      >{{ $t('account.next') }}<ArrowLongRightIcon :class="[pagination.links.next ? 'text-gray-400' : 'text-gray-300']" class="ml-3 size-5" aria-hidden="true" />
      </button>
    </div>
  </nav>
</template>
