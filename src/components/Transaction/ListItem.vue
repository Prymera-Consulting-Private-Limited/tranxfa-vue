...
<template>
  <div :class="{'opacity-75': transaction.state.code === TransactionState.CANCELLED}" class="flex min-w-0 gap-x-4 px-4 md:px-0">
    ...
    <div class="min-w-0 flex-auto">
      <i18n-t keypath="account.amountToRecipient" tag="div" class="text-sm/6 font-semibold text-gray-900" scope="global">
      ...
      <div class="text-xs/5 text-gray-800 flex justify-start items-center gap-x-1.5">{{ $t('account.sentAmountViaMethod', {foreignAmountCurrencyPrefixed: transaction.foreignAmountCurrencyPrefixed, title: transaction.payoutMethod.title}) }}<span class="flex justify-center items-center text-xs/5 text-gray-500">
