  ...
  <div v-if="walletStore.isEnrolled" class="rounded-lg bg-white shadow-lg p-5">
    <div class="flex items-center justify-between">
      <div class="flex items-center gap-x-3">
        ...
        <div>
          <p class="text-xs/5 font-medium text-gray-500">{{ $t('transfer.wizard.walletBalance') }}</p>
          ...
          <p v-else class="text-lg font-bold tracking-tight text-gray-900">{{ $t('wallet.mdash') }}</p>
        </div>
      </div>
      <router-link :to="{name: 'wallet'}" class="shrink-0 text-sm/6 font-semibold text-brand-700 hover:text-brand-800">{{ $t('wallet.manage') }} <span aria-hidden="true">→</span></router-link>
    </div>
  </div>
</template>
