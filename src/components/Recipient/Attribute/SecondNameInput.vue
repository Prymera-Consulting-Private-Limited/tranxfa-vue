...
<template>
  <TextInput v-if="! disableNameInput" v-on:recipient:input:updated="secondNameUpdated" :id="id" class="block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none" />
  <div v-else class="block w-full px-3 py-2 border border-gray-300 rounded-md bg-gray-50">{{ attribute.attribute }}</div>
</template>