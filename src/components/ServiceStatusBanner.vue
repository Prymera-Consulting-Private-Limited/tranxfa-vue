  ...
  <div v-if="activeIsBlocking" role="alert" class="border-b border-warning-200 bg-warning-50">
    <div class="mx-auto flex max-w-7xl items-start gap-3 px-4 py-3 sm:px-6 lg:px-8">
      ...
      <div class="text-sm/6 text-warning-800">
        <p class="font-semibold">{{ active.message || 'Estamos realizando tareas de mantenimiento en este momento.' }}</p>
        <p v-if="active.expectedToEndAt">Esperamos volver alrededor de las {{ when(active.expectedToEndAt) }}. Volveremos a comprobarlo por ti.</p>
        <p v-else>Volveremos a comprobarlo en breve.</p>
      </div>
    </div>
  </div>
  <div v-else-if="upcoming" role="status" class="border-b border-info-200 bg-info-50">
    <div class="mx-auto flex max-w-7xl items-start gap-3 px-4 py-3 sm:px-6 lg:px-8">
      ...
      <div class="text-sm/6 text-info-800">
        <p class="font-semibold">Mantenimiento programado{{ upcoming.startsAt ? ` desde ${when(upcoming.startsAt)}` : '' }}{{ upcoming.expectedToEndAt ? ` hasta aproximadamente ${when(upcoming.expectedToEndAt)}` : '' }}.</p>
