...
<template>
  ...
  <template v-else-if="status === 'pending'">
    ...
    <p v-if="isSlow" role="status" class="mt-2 rounded-lg border border-warning-200 bg-warning-50 px-4 py-3 text-left text-sm/6 text-warning-800"><i18n-t keypath="payment.provider.thisIsTakingLongerThan5" scope="global"><template #value><router-link :to="{name: 'viewTransaction', params: {transactionId: transaction.id}}" class="font-semibold underline underline-offset-2">{{ $t('payment.card.goToYourTransfer') }}</router-link></template></i18n-t></p>
  </template>
  ...
  <template v-else-if="status === 'processing'">
    ...
    <p class="text-base text-gray-600 mb-6">{{ transaction.payment.clientPaymentAccount?.waitTimeMessage || $t('payment.provider.thisUsuallyTakesUnderA') }}</p>
