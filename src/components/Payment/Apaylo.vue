...
<template>
  <template v-if="transaction.payment.state.code === PaymentState.PENDING">
    <div class="-m-5 -mt-10">
      ...
      <a :href="transaction.payment.paymentUrl" @click="redirectToPaymentUrl" target="_blank" class="block w-full px-4 md:px-6 lg:px-8 bg-success-600 text-white text-center py-3 rounded-md font-medium hover:bg-success-700 transition cursor-pointer text-sm/6 outline-none ring-0 tracking-wider focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-brand-700">Pay {{ transaction.payment.totalPaymentAmountCurrencyPrefixed }}</a>
  ...
  <template v-else-if="status === 'failed'">
    ...
    <button @click="retryPayment" class="mt-5 px-4 md:px-6 block w-full lg:px-8 bg-brand-600 text-white text-center py-3 rounded-md font-medium hover:bg-brand-800 transition cursor-pointer text-sm/6 outline-none ring-0 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-brand-700">Retry Payment</button>
    <p class="text-base text-danger-600 mt-5 text-sm/6">If the issue continues, please contact our support team. We'll be happy to assist you!</p>
  </template>
</template>