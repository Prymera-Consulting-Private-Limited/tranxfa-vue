...
<template>
  ...
  <template v-else-if="status === 'failed'">
    ...
    <template v-if="transaction.payment.paymentProvider.paymentDataAttributes?.length > 0">
      ...
      <template v-for="attribute in transaction.payment.paymentProvider.paymentDataAttributes">
        <div class="mb-3 text-left">
          ...
          <p v-if="retryFormErrors[`${attribute.attribute}`]?.length > 0" class="mt-2 text-sm text-danger-600">{{ retryFormErrors[`${attribute.attribute}`][0] }}</p>
        </div>
      </template>
    </template>
    ...
    <p class="text-base text-danger-600 mt-5 text-sm">If the issue continues, please contact our support team. We'll be happy to assist you!</p>
  </template>
</template>