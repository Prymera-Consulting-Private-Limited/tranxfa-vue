...
<template>
  <div>
    ...
    <p class="mt-1 flex items-baseline gap-1.5">
      <span class="text-sm/6 font-medium text-gray-500">{{ money.currency }}</span>
      <span class="text-2xl font-semibold tracking-tight text-gray-900">{{ rate.total.formatted }}</span>
    </p>
    <p v-if="rate.perNight.isStated" class="mt-0.5 text-xs/5 text-gray-500">{{ nights }} night{{ nights === 1 ? '' : 's' }} &middot; {{ rate.perNight.currencyPrefixed }} / night</p>
    <!-- What the total is made of -->
    <dl v-if="showBreakdown" class="mt-2 space-y-0.5">
      <div v-for="line in lines" :key="line.key" class="flex items-baseline justify-between gap-2 text-xs/5">
    ...
    <p v-if="rate.payableAtProperty.isStated" class="mt-2 text-xs/5 text-warning-700">Plus {{ rate.payableAtProperty.currencyPrefixed }} payable at the property</p>
  </div>
</template>
