  ...
  <form @submit.prevent="submit" class="overflow-hidden rounded-3xl bg-white ring-1 ring-gray-200">
    ...
    <div class="space-y-5 px-5 py-5">
        ...
        <div v-for="(guest, guestIndex) in room.guests" :key="guestIndex">
          ...
          <div class="mt-2 grid grid-cols-1 gap-3 sm:grid-cols-2">
            <div>
              ...
              />
              <p v-if="fieldError(`rooms.${roomIndex}.guests.${guestIndex}.first_name`)" class="mt-1 text-xs/5 text-danger-600">{{ fieldError(`rooms.${roomIndex}.guests.${guestIndex}.first_name`) }}</p>
            </div>
            <div>
              <input
              ...
              />
              <p v-if="fieldError(`rooms.${roomIndex}.guests.${guestIndex}.last_name`)" class="mt-1 text-xs/5 text-danger-600">{{ fieldError(`rooms.${roomIndex}.guests.${guestIndex}.last_name`) }}</p>
            </div>
          </div>
        </div>
      </div>
      <div class="border-t border-gray-100 pt-5">
        ...
        <div class="mt-2 grid grid-cols-1 gap-3 sm:grid-cols-2">
          <div>
            ...
            />
            <p v-if="fieldError('email')" class="mt-1 text-xs/5 text-danger-600">{{ fieldError('email') }}</p>
          </div>
          <div>
            <input
                v-model="phone"
            ...
            />
            <p v-if="fieldError('phone')" class="mt-1 text-xs/5 text-danger-600">{{ fieldError('phone') }}</p>
          </div>
        </div>
      </div>
      <div v-if="submitError" class="flex items-start gap-2 rounded-xl border border-danger-200 bg-danger-50 p-3 text-sm/6 text-danger-700">
      ...
      >
        ...
        {{ isSubmitting ? $t('travel.preparingYourBooking') : $t('travel.continueToPayment') }}
      </button>
    </div>
  </form>
</template>
