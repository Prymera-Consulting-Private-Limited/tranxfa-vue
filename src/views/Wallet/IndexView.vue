      ...
      <div class="mx-auto max-w-3xl px-4 sm:px-6 lg:max-w-7xl lg:px-8">
        ...
        <template v-else-if="! walletStore.isEnrolled">
          <div class="mx-auto max-w-3xl rounded-lg bg-white border border-gray-100 p-8 sm:p-12 text-center">
            ...
            <p class="mx-auto mt-2 max-w-md text-sm/6 text-gray-500">Load money into your wallet by bank transfer and pay for transfers instantly — no waiting on your bank at checkout, and refunds come straight back to your balance.</p>
            <div class="mx-auto mt-8 grid max-w-2xl grid-cols-1 gap-4 text-left sm:grid-cols-3">
              <div class="rounded-lg border border-gray-200 p-4">
                ...
                <p class="mt-2 text-sm/6 font-medium text-gray-900">Load once, send many times</p>
              </div>
              <div class="rounded-lg border border-gray-200 p-4">
                <PlusIcon class="size-6 text-brand-600" aria-hidden="true" />
                <p class="mt-2 text-sm/6 font-medium text-gray-900">Pay for transfers instantly at checkout</p>
              </div>
              <div class="rounded-lg border border-gray-200 p-4">
                <ArrowPathIcon class="size-6 text-brand-600" aria-hidden="true" />
                <p class="mt-2 text-sm/6 font-medium text-gray-900">Refunds return straight to your wallet</p>
              </div>
            </div>
          ...
          <div class="grid gap-8 lg:grid-cols-3">
            <div class="flex flex-col gap-4 lg:col-span-2">
              ...
              <div class="rounded-lg bg-white border border-gray-100 px-4 py-5 sm:px-6">
                <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                  <div>
                    <h2 class="text-sm/6 font-medium text-gray-500">Wallet balance</h2>
                    ...
                    <UseClipboard v-slot="{ copy, copied }" :source="walletStore.wallet.data?.walletNumber ?? walletStore.subscription.data?.walletNumber">
                      <p class="mt-2 flex items-center gap-x-1.5 text-sm/6 text-gray-500">
                        ...
                        <span v-if="copied" class="text-xs/5 text-success-700">Copied!</span>
                      </p>
                    </UseClipboard>
                  </div>
                  <div class="flex shrink-0 flex-col gap-2 sm:items-end">
                    ...
                    <router-link :to="{name: 'dashboard'}" class="inline-flex items-center justify-center rounded-xl bg-white px-4 py-2.5 text-sm/6 font-semibold text-gray-900 ring-1 ring-inset ring-gray-300 transition hover:bg-gray-50">Send money</router-link>
                  </div>
                </div>
              </div>
              ...
              <div class="rounded-lg bg-white border border-gray-100 px-4 py-4 sm:px-6">
                <div class="flex items-center justify-between">
                  ...
                  <router-link :to="{name: 'walletStatement'}" class="text-sm/6 font-semibold text-brand-700 hover:text-brand-800">View statement &rarr;</router-link>
                </div>
                ...
                <template v-else>
                  <p class="mt-3 py-6 text-center text-sm/6 text-gray-500">No movements yet — add money to get started.</p>
                </template>
              </div>
            </div>
            ...
            <div>
              <div class="rounded-lg bg-white border border-gray-100 p-5">
                ...
                <p class="mt-4 text-xs/5 text-gray-500">A declaration stays open for 72 hours. A deposit that arrives late or with a different amount isn't lost — our support team takes care of it.</p>
              </div>
            </div>
          </div>
        </template>
