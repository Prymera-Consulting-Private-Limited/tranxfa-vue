              ...
              <div>
                ...
                <div class="mt-6 border-t border-b border-gray-200 py-6 w-full">
                  <ul v-if="customerStore.isLoaded === true" role="list" class="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
                    <li v-for="document in customerStore.customer.documents" :key="document.id" class="col-span-1 flex flex-col divide-y divide-gray-200 rounded-lg bg-green-300/20 border border-green-800 text-center shadow-sm">
                    ...
                    <li v-for="pendingCategory in customerStore.customer.pendingDocuments" :key="pendingCategory.id" class="col-span-1 flex flex-col divide-y divide-gray-200 rounded-lg bg-white text-center shadow-sm">
                      <div class="flex flex-1 flex-col p-8">
                        ...
                        <dl v-if="pendingCategory.description" class="mt-1 flex grow flex-col justify-between">
                          ...
                          <dd class="mt-3 text-sm text-gray-500">
                            <router-link :to="{name: 'documentTypeSelectionForUpload', params: {category: pendingCategory.id}}" class="text-brand-700 font-semibold hover:underline">Start Verification</router-link>
                          </dd>
                        </dl>
                      </div>
                    </li>
