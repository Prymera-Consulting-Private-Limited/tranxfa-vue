...
async function getSources () {
  countryUtils.getSources().finally(() => {
...
<template>
  <main>
    <div class="flex items-center justify-center min-h-screen bg-gray-50 tracking-wider">
      <i v-if="isLoading && countryUtils.sources.value" class="pi pi-spin pi-spinner text-5xl text-purple-700"></i>
      <div v-else class="relative flex flex-col md:flex-row w-full h-screen bg-white">
        ...
        <div class="flex-1 flex items-center justify-center p-4 md:p-8">
          <div class="w-full max-w-xl">
            ...
            <ul v-for="country in countryUtils.sources.value" :key="country.id" role="list" class="mt-3 grid grid-cols-1 gap-5 lg:grid-cols-2 sm:gap-6">
