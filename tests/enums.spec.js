...
describe('enums', () => {
    ...
    describe('PaymentState', () => {
        // Two states do not mirror their key. The API spells them with a
        // hyphen, and main corrected the enum to match the wire in 29fb0cc -
        // a value of 'TIMED_OUT' simply never matches what arrives. This
        // branch was cut before that, so the old blanket key === value
        // invariant is what fails here, not the enum.
        const spelledWithAHyphen = {
            TIMED_OUT: 'TIMED-OUT',
            PART_REFUNDED: 'PART-REFUNDED',
        }

        it('mirrors its key except where the wire spelling differs', () => {
            for (const [key, value] of Object.entries(PaymentState)) {
                expect(value).toBe(spelledWithAHyphen[key] ?? key)
            }
        })

        it('keeps the two hyphenated states hyphenated', () => {
            expect(PaymentState.TIMED_OUT).toBe('TIMED-OUT')
            expect(PaymentState.PART_REFUNDED).toBe('PART-REFUNDED')
        })
